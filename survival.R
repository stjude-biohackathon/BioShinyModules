...
survival_demo <- function() {
        ...
        server <- function(input, output, session) {
                survival_server("x", reactive({df}), reactive({time_var}), reactive({event_var}),
                                reactive({group_var}), reactive({group_var_type}) )
        }
        shinyApp(ui, server)
}
...
survival_demo_1 <- function() {
        ...
        server <- function(input, output, session) {
                ...
                survival_server("x", df, reactive({input$time_var}), reactive({input$event_var}),
                                reactive({input$group_var}), reactive({input$group_var_type}) )
        }
        shinyApp(ui, server)
}
...
survival_demo_2 <- function() {
        ...
        server <- function(input, output, session) {
                ...
                my_plot <- survival_server("x", df, reactive({input$time_var}), reactive({input$event_var}),
                                           reactive({input$group_var}), reactive({input$group_var_type}) )
